...
import { Framework as SuperfluidFramework, IWeb3FlowInfo } from "@superfluid-finance/sdk-core";
import { useWeb3React } from "@web3-react/core";
import { useEffect, useState } from "react";

export function ProfileSuperfluid({ address }: { address: string }) {
  const { provider } = useWeb3React();
  const [superfluid, setSuperfluid] = useState<SuperfluidFramework | undefined>(undefined);
  ...
  if (!superfluid)
    return <div>Please sign in using Polygon Network to open a stream for this profile</div>;
  ...
  return (
    <>
      {existingFlow ? (
        ...
        <button onClick={() => setModalOpen(true)}>Sponsor (create stream)</button>
      )}
      {modalOpen && (
        <div
