...
export function Stream({ stream, stopStream }: { stream: any; stopStream: any }) {
  ...
  return (
    <div className="p-4 m-4 mx-auto bg-white shadow-xl rounded-xl">
      ...
      {Number(stream.currentFlowRate) > 0 && !canceled ? (
        <div>
          <div className="break-words">Current flowrate: {stream.currentFlowRate}</div>
          <button
        ...
        <div className="break-words"> Total streamed: {stream.streamedUntilUpdatedAt}</div>
      )}
