...
export function Discord() {
  ...
  const connectDiscord = async () => {
    ...
    const request = await fetch("https://f3bridge.experimental.xeno.yt/api/v1/auth", {
      body: JSON.stringify({
    ...
    const verify = await fetch("https://f3bridge.experimental.xeno.yt/api/v1/auth/verify", {
      body: JSON.stringify({
        ...request,
        response,
      }),
    ...
    const discordLink = await fetch("https://f3bridge.experimental.xeno.yt/api/v1/discord", {
  ...
  return (
    <div className="max-w-3xl m-auto my-4 px-4">
      <button onClick={connectDiscord}>Connect Discord to view your friend recommendations</button>
    </div>
  );
}
